...
# NEW: block caching
from src.model.llada_v2.block_utils import BlockCacheV2 as BlockCache
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        use_block_caching: bool = False,
        block_size: Optional[int] = None,
        save_cache: bool = False,
        clean_idx: Optional[int] = None,
        use_full_query_attn: bool = False,
    ) -> Tuple[torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]]]:
        B, T, C = q.size()  # batch size, sequence length, d_model
        dtype = k.dtype

        # assert use_block_caching, "Block caching is forced to be true for now"
        ...
        if use_block_caching:
            assert self.block_cache is not None, "Block cache is not initialized"
            assert block_size is not None, "Block size is not provided"
            assert save_cache is not None, "Save cache is not provided"
            assert use_full_query_attn is not None, "Use full query attention is not provided"


            # TODO: Implement block caching
            # new k, q, v is passed in
            new_k = k # (B, num_kv_heads, T, head_dim)
            new_v = v # (B, num_kv_heads, T, head_dim)
            new_q = q # (B, num_heads, T, head_dim)
            
            # flatten the head_dim dimension

            # check new_q, new_k, new_v dimensions (layer 0)
            # if self.layer_id == 0:
            ...
            # update cache
            self.block_cache.update_cache(new_q, new_k, new_v)
            # save cache if needed
            if save_cache:
                self.block_cache.save_cache(clean_idx)

            # get cache (returns all cached tensors, up to next_cache_idx)
            full_q, full_k, full_v, _, _ = self.block_cache.get_cache()

            # if self.layer_id == 0:
            #     print(f"full_q shape: {full_q.shape}")
            #     print(f"full_k shape: {full_k.shape}")
            #     print(f"full_v shape: {full_v.shape}")
            ...
            if self.config.rope:
                ...
                full_q, full_k = self.rotary_emb(full_q, full_k)
                # truncate to the query length (last T tokens)
                new_q = full_q[:, :, -T:, :]
            ...
            #     # use all query tokens for attention
            #     q = full_q
            ...
            if use_full_query_attn:
                raise NotImplementedError("Full query attention is not implemented for block caching")
            ...
            # raise NotImplementedError("Block caching is not implemented")
    ...
    def reset_block_cache(self, batch_size: int, max_length: int, block_size: int):
        self.block_cache = BlockCache(
            ...
            # block_size=block_size, # not used in BlockCacheV2 
            num_heads=self.config.n_heads,
            num_key_value_heads=self.config.effective_n_kv_heads,
...
class LLaDAModel(nn.Module):
    ...
    def reset_block_cache(self, batch_size: int, max_length: int, block_size: int):
        if self.config.block_group_size == 1:
            for block in self.transformer.blocks:
                block.reset_block_cache(batch_size, max_length, block_size)
        else:
            for block_group in self.transformer.block_groups:
    ...
    def forward(
        ...
        if use_block_caching and not self.cache_initialized:
            batch_size = input_ids.shape[0] if input_embeddings is None else input_embeddings.shape[0]
            max_length = self.config.max_sequence_length
            self.reset_block_cache(batch_size, max_length, block_size)
...
class LLaDAV2ModelLM(PreTrainedModel):
    """
    Extremely barebones HF model wrapper.
    """

    config_class = LLaDAV2Config
    base_model_prefix = "model"
    _no_split_modules = ["LLaDABlock", "LLaDASequentialBlock", "LLaDALlamaBlock"]

    def __init__(self, config: LLaDAV2Config, model: Optional[BlockCachedLLaDA] = None, init_params: bool = False):
        ...
        # print(f"Initializing LLaDAV2ModelLM with config: {config}")
        print(f"{BOLD}{GREEN}Initializing LLaDAV2ModelLM with config: {config}{RESET}")
...
AutoModel.register(LLaDAV2Config, LLaDAV2ModelLM)