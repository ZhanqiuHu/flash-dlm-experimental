...
class Attention(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.wq = nn.Linear(config.d_model, config.d_model, bias=config.include_bias or config.include_qkv_bias)
        self.wk = nn.Linear(config.d_model, self.n_kv_heads * self.head_dim, bias=config.include_bias or config.include_qkv_bias)
        self.wv = nn.Linear(config.d_model, self.n_kv_heads * self.head_dim, bias=config.include_bias or config.include_qkv_bias)
        self.wo = nn.Linear(config.d_model, config.d_model, bias=config.include_bias)

        if config.rope:
            self.rotary_emb = RotaryEmbedding(config, BufferCache())
    ...
    def attention(
        ...
        # Zhanqiu modified: Store raw attention weights for analysis.
